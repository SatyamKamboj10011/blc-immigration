    ...
    >
      <div className="relative aspect-video w-full overflow-hidden bg-brand-950">
        ...
        <span className="absolute inset-0 flex items-center justify-center bg-brand-950/25 transition-colors group-hover:bg-brand-950/10">
          <span className="flex h-11 w-11 items-center justify-center rounded-full bg-gold-500 text-brand-950 shadow-md">
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-5">
        <h3 className="font-display text-base font-extrabold uppercase tracking-tight text-brand-950">{name}</h3>
        <p className="text-sm leading-relaxed text-brand-500">{outcome}</p>
        <span className="eyebrow mt-auto pt-2 text-gold-700">Watch on YouTube</span>
      </div>
    </a>
  );
}
